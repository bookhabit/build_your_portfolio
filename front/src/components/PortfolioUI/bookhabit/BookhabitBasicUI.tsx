...
const BookhabitBasicUI = ({portfolio,userPage}:IProps) => {
        ...
        // 포트폴리오의 form에 채워진 데이터를 UI에 넘겨준다
          return (
            ...
            <div className="bg-black p-8 grid justify-center gap-4">
              ...
              <BookhabitImageUI src={showPreviewSrc} alt="이미지 전체보기" />
            </div>
          </div>
          );
    } 
                ...
                <div className="flex flex-col lg:flex-row gap-20 mb-12">
                    ...
                    <div className="w-full lg:w-1/2 shadow-xl">
                        {portfolio?.PortfolioDoc.photos.length > 0 && 
                            <BookhabitImageUI className="w-full aspect-square object-fill" src={portfolio?.PortfolioDoc.photos[0]}
                            alt="포트폴리오 이미지"
                            />
                        }
                    </div>
                    ...
                    <div className="flex flex-col gap-3 my-12 border shadow-xl p-10" key={importantData.important_function_photo[0]}>
                        ...
                        <div className="flex flex-col gap-10 lg:flex-row">
                            ...
                            onClick={()=>{
                                ...
                                <BookhabitImageUI className=" h-80 object-fill border shadow-lg cursor-pointer hover:shadow-2xl" src={importantData.important_function_photo[0]}
                                alt="핵심기능 이미지"
                                />
                            </div>
                    ...
                    <div className=" w-2/3 grid grid-cols-3 gap-5">
                        {portfolio.PortfolioDoc.photos.length>1 && portfolio.PortfolioDoc.photos.slice(1).map((photo)=>(
                            <div key={photo} className="w-full" onClick={()=>{
                                ...
                                <BookhabitImageUI className="h-full object-cover border shadow-lg cursor-pointer hover:shadow-2xl"  src={photo} alt="포트폴리오 이미지" />
                            </div>
                        ))}
                    </div>
                </div>
...
export default BookhabitBasicUI;
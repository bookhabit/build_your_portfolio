...
type PhotosUploaderProps = {
  addedPhotos: string[]; // addedPhotos의 타입은 문자열 배열입니다.
    ...
    <>
      <div className="flex gap-5 h-11">
        <input value={photoLink}
               ...
               type="text" placeholder={'이미지 링크 업로드'}
               className="w-full p-2 border border-gray-500 rounded-lg"
               />
        <button onClick={addPhotoByLink} className="bg-transparent border border-gray-500 px-4 rounded-lg font-bold hover:text-white min-w-fit hover:bg-gray-300">추가</button>
      </div>
